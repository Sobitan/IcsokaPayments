...
namespace IcsokaPayments.Service.Helpers
{
    public class WebHelper:IWebHelper
    {
        ...
        {
            return _httpContext.Request.Url != null ? _httpContext.Request.Url.GetLeftPart(UriPartial.Authority) : string.Empty;
        }

        public string GetBackgroundTaskHost(bool useSsl)
        {
            var result = "";
        ...
        public string GetBackgroundTaskLocation()
        {
            bool useSsl = IsCurrentConnectionSecured();
            return GetBackgroundTaskLocation(useSsl);   
        }

        public string GetBackgroundTaskLocation(bool useSsl)
        {
            string result = GetBackgroundTaskHost(useSsl);
            if (result.EndsWith("/"))
        ...
        public string GetHostPart(string url)
        {
            if (!CommonHelper.HasValue(url)) return string.Empty;
            var uri = new Uri(url);
        ...
        public static HttpWebRequest CreateHttpRequestForSafeLocalCall(Uri requestUri)
        {
            ...
            request.UserAgent = "VATCOLLECT_v2";
